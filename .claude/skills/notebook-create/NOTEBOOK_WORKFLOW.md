...
If you are creating or editing a **course notebook**, read these two files before doing anything else:

- [`../references/course_intro.md`](../references/course_intro.md) — course identity, learning objectives, tech stack defaults, and structural overview
- [`../references/course_syllabus.md`](../references/course_syllabus.md) — chapter-by-chapter breakdown with lesson titles, prerequisites, and notebook naming conventions

These files tell you which chapter the notebook belongs to, what students already know at that point, what concepts come next, and what tech stack is expected. Without this context, notebooks risk teaching the wrong model, using the wrong framework, or assuming knowledge students don't yet have.
...
- Common API keys include `GOOGLE_API_KEY`, `OPENAI_API_KEY`, `ANTHROPIC_API_KEY`, and `TOGETHER_API_KEY`. Check `../references/course_intro.md` and the notebook's imports to determine which keys are needed — always load them from `.env` or Colab Secrets, never hardcoded.
- For local testing, set env vars in the shell before running: `export OPENAI_API_KEY="..."` etc.
