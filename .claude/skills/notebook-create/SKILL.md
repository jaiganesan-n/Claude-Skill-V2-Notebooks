...
### 0. Load Context and Per-Notebook Instructions

Read [`../references/course_intro.md`](../references/course_intro.md) and [`../references/course_syllabus.md`](../references/course_syllabus.md) to confirm the notebook's chapter, the expected tech stack for that chapter, and what prerequisites students bring. This determines which frameworks and models to use.

**Per-notebook instructions take precedence over all defaults.**
If the user has provided targeted instructions for this notebook (specific model, framework, libraries, scope, style, or structure), those instructions override the defaults in course_intro.md, course_syllabus.md, and NOTEBOOK_WORKFLOW.md. Apply defaults only for anything the user has NOT explicitly specified.

Before writing any code, confirm your understanding with the user if anything in the per-notebook instructions conflicts with course defaults (e.g. a model not in the standard stack, a library version different from colab_libraries.txt). State the conflict and your intended resolution in one sentence — don't silently override, and don't silently comply without noting the deviation.
...
6. Name the file to closely match the lesson title from `../references/course_syllabus.md` for the relevant chapter (e.g., a lesson titled "Hybrid Search" → `hybrid_search.ipynb`)
...
Once restored, determine the output path and convert. Default to saving inside the `notebooks/` subdirectory (creating it if needed) so the file is immediately findable by `notebook-edit`:

```bash
# Save inside notebooks/ subdirectory; auto-increment version suffix if file already exists
mkdir -p "$PROJECT_DIR/notebooks"
BASE="$PROJECT_DIR/notebooks/<notebook_name>"
OUTPUT="${BASE}.ipynb"
VERSION=2
while [ -f "$OUTPUT" ]; do
...
```bash
# 0. Read course_intro.md + course_syllabus.md
#    Apply any per-notebook instructions from the user — these override defaults.
...
# 11. Determine output path (notebooks/ subdir, auto-increment if exists) and convert
mkdir -p "$PROJECT_DIR/notebooks"
BASE="$PROJECT_DIR/notebooks/<notebook_name>"; OUTPUT="${BASE}.ipynb"; VERSION=2
